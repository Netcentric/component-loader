...
import { loaderInstances } from './loaderInstances';
import { loaderCreateComponent } from './loaderCreateComponent';
...
export const loaderRun = (element = window.document, initAttr = 'data-nc') => {
  ...
  if (element.hasAttribute && element.hasAttribute(this.attribute)) {
    nodes.push(element);
  }
  ...
  nodes.forEach(async (node) => {
    ...
    if (loaderComponents[name]) {
      ...
      loaderInstances[name].push(loaderCreateComponent(
